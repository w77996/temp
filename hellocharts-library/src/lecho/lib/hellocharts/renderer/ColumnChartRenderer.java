...
public class ColumnChartRenderer extends AbstractChartRenderer {
    ...
    private void calculateMaxViewport() {
        ...
        tempMaximumViewport.set(-0.5f, baseValue, data.getColumns().size(), baseValue);
        if (data.isStacked()) {
...
//        canvas.drawRect(drawRect, columnPaint);
        canvas.drawRoundRect(drawRect, drawRect.width() / 10, drawRect.width() / 10, columnPaint);
        if (column.hasLabels()) {
    ...
    private void highlightSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, int valueIndex,
        ...
        if (selectedValue.getSecondIndex() == valueIndex) {
            ...
            canvas.drawRect(drawRect.left - touchAdditionalWidth, drawRect.top, drawRect.right + touchAdditionalWidth, drawRect.bottom, columnPaint);
